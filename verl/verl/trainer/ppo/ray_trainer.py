...
class RayPPOTrainer(object):
    ...
    def _validate(self):
        ...
        for data_source, rewards in data_source_reward.items():
            metric_dict[f'val/test_acc/{data_source}'] = np.mean(np.where(np.array(rewards) <= 0, 0, 1))
    ...
    def fit(self):
        ...
        self.lambda_len = self.config.algorithm.length_ctrl.lambda_init
        self.acc_baseline = self.config.algorithm.length_ctrl.acc_init
            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer('step', timing_raw):
                    ...
                    with _timer('adv', timing_raw):
                        ...
                        seq_acc   = (self._orig_reward_fn(batch).sum(-1) > 0).float().mean()
                        
                        self.acc_baseline = self.config.algorithm.length_ctrl.acc_init
                        gap       = seq_acc -self.acc_baseline 
                        self.lambda_len = max(0.0, self.lambda_len + self.config.algorithm.length_ctrl.lr * gap)
                        metrics["lagrange/lambda"] = self.lambda_len
                        metrics["lagrange/gap"]    = gap.item()
                        metrics["lagrange/acc"]    = self.acc_baseline
                        metrics["lagrange/seq_acc"] = seq_acc.item()
