...
def deepscaler_reward_fn(solution_str: str, ground_truth: Union[str, List[str]], enable_llm = False, num_tokens = -1, valid_response_length = -1, ignore_think_token = False, max_token_constraint = -1):
    reward_config = RewardConfig()
    reward_config.use_math_orm = enable_llm
    ...
    elif max_token_constraint != -1:
        correctness_score = 0 if not reward_response.is_correct else 1
        
        return max_token_constraint * correctness_score - float(valid_response_length)
    else:
        correctness_score = 0 if not reward_response.is_correct else 1
